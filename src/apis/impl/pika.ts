...
async function handleBedwarsCall(playerName: string): Promise<BedwarsMode> {
    const bedwarsProfile = await pika.getProfileLeaderboard({
        username: playerName,
        gamemode: (Pika as any).PikaNetworkGamemode.BedWars,
        mode: (Pika as any).PikaNetworkMode.AllModes,
        interval: (Pika as any).PikaNetworkInterval.AllTime,
    });
...
async function handleGlobalCall(playerName: string): Promise<GlobalMode> {
    const profile = await pika.getProfile(playerName);
